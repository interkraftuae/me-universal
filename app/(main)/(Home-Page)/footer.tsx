...
const FooterColumn = ({ title, links }: { title: string; links: string[] }) => (
  <div>
    <h4 className="text-md font-montserrat tracking-tight font-bold text-primary uppercase mb-2">
      {title}
    </h4>
    <ul className="space-y-3 font-roboto">
      {links.map((link) => (
        <li key={link}>
          <a
            href="#"
            className="text-xs text-gray-600 uppercase tracking-wide transition-colors hover:text-primary"
          >
            {link}
          </a>
        </li>
...
export default function Footer() {
  return (
    <footer className="border-t border-gray-200 mt-16 bg-white pt-12 pb-6 px-6 md:px-16">
      <div className="container">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-10 mb-10">
          {/* Brand Column */}
          <div className="flex flex-col gap-4 col-span-2">
            <div>
              <Logo mode="light" className="w-40" />
            </div>
        ...
        <div className="border-t border-gray-100 pt-5 flex flex-col md:flex-row justify-between items-center gap-2">
          ...
          <p className="text-xs text-gray-400 font-[500] tracking-widest uppercase">
            Created by{" "}
            <a
